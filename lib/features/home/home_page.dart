...
import '../trends/primex_trends_page.dart';
import '../../widgets/primex_drawer_menu.dart';
import 'primex_home_item_details_page.dart';
...
class HomePage extends StatelessWidget {
  ...
  void openBrowseTab(BuildContext context, String tab) {
    ...
    if (tab == 'Service' || tab == 'Services' || tab == 'Jobs/Services') {
      Navigator.push(
        context,
        MaterialPageRoute(builder: (_) => const JobsServicesPage()),
      );
      return;
    }
          ...
          child: Row(
            ...
            children: [
              FloatingActionButton.extended(
                ...
                heroTag: 'primex_trends_fab',
                onPressed: () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (_) => const PrimeXTrendsPage(),
                    ),
                  );
                },
              ...
              const PrimeXAiFloatingButton(
                module: 'home',
                heroTag: 'primex_home_ai_fab',
              ),
            ],
          ),
        ),
      ),
      drawer: const PrimeXDrawerMenu(),
          ...
          children: [
            ...
            Wrap(spacing: 8, runSpacing: 8, children: [
              ...
              GestureDetector(
                ...
                child: bubble(Icons.handyman, 'Services'),
              ),
            ...
            // Home feed removed - now shown only in Trends
...
class PrimeXHomeUnifiedMarketplaceFeed extends StatelessWidget {
  const PrimeXHomeUnifiedMarketplaceFeed({super.key});

  Future<List<Map<String, dynamic>>> loadItems() async {
    final items = <Map<String, dynamic>>[];

    Future<void> pull(String col, String type) async {
      try {
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<List<Map<String, dynamic>>>(
      ...
      builder: (context, snap) {
        ...
        final items = rawItems.where((x) {
          final type = (x['_type'] ?? x['type'] ?? '').toString().toLowerCase();
          final isAd = type.contains('ad') ||
              type.contains('banner') ||
              type.contains('promotion');
          if (!isAd) return true;

          final paid = x['paymentStatus'] == 'paid' || x['paid'] == true;
          final approved = x['approved'] == true || x['status'] == 'approved';
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 8),
              if (items.isEmpty)
                const Text('Posts, listings, ads and banners will appear here.',
                ...
                Wrap(
                  ...
                  children: items.map((d) {
                    ...
                    final name = (d['displayName'] ??
                        ...
                        .toString();
                    final avatar =
                        (d['photoUrl'] ?? d['profilePhoto'] ?? '').toString();
