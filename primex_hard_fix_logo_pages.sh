...
cp lib/main.dart "lib/main_backup_hard_fix_logo_pages_$(date +%Y%m%d_%H%M%S).dart"
...
class _PrimeXHomeState extends State<PrimeXHome> {
  ...
  Widget howItWorks() => futurePage(
        ...
        [
          ...
          card('🛡', 'AI Autopilot Safety',
              'AI monitors scams, fraud, harassment, discrimination, nudity, sexual solicitation, fake listings, spam, bots, and suspicious activity.'),
          card('🔐', 'Security Shield',
              'Security Shield supports account protection, suspicious login review, identity protection, bot detection, and admin alerts.'),
        ],
      );
  ...
  Widget pricing() => futurePage(
        'Pricing',
        'Simple PrimeX pricing for listings, boosts, leads, and PrimeX Pro.',
        [
  ...
  Widget contact() => futurePage(
        'Contact',
        'Reach PrimeX Marketplace and Syntax Phantom support.',
        [
  ...
  Widget login() => futurePage(
        'Login',
        'Access the PrimeX member dashboard.',
        [field('Email'), field('Password'), fullBtn('Login To Dashboard', () => go(7))],
      );

  Widget signup() => futurePage(
        'Sign Up',
        'Create your PrimeX account.',
        [
          field('Full Name'),
          field('Email'),
          field('Phone'),
  ...
  Widget dashboard() => futurePage(
        'User Dashboard',
        'Marketplace tools live here after login.',
        [
          dashTile('🛒', 'Marketplace', 'Browse listings, save opportunities, message sellers, call, and make offers.', () => go(8)),
          dashTile('🗺', 'Map', 'View category pins. PrimeX Pro unlocks foreclosure and tax-sale lead details.', () {}),
          dashTile('➕', 'Post a Listing', 'Create posts for real estate, rentals, vehicles, services, jobs, tools, and business opportunities.', () {}),
          dashTile('💬', 'Messages', 'Secure buyer and seller messaging inside PrimeX.', () {}),
          dashTile('💰', 'Offers + Proof of Funds', 'Submit serious offers and upload proof of funds.', () {}),
          dashTile('📊', 'Admin Access', 'Admin-only moderation, AI alerts, analytics, revenue, and security tools.', () {}),
        ],
      );

  Widget marketplace() => futurePage(
        'Marketplace Inside Dashboard',
  ...
  Widget dashTile(String icon, String title, String body, VoidCallback onTap) =>
      InkWell(onTap: onTap, child: card(icon, title, body));
...
class PrimeXLogoPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final rect = RRect.fromRectAndRadius(
      Rect.fromLTWH(0, 8, size.width - 8, size.height - 16),
    ...
    prime.paint(canvas, const Offset(20, 10));
    ...
    market.paint(canvas, const Offset(22, 54));
    ...
    tag.paint(canvas, const Offset(22, 71));

    final ai = RRect.fromRectAndRadius(
      Rect.fromLTWH(size.width - 82, 24, 56, 46),
    ...
    aiText.paint(canvas, Offset(size.width - 62, 36));
  }
...
class HeroPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawRect(
      ...
      Paint()
        ..shader = const LinearGradient(
          colors: [Colors.black, Color(0xFF08256B), Colors.black],
        ).createShader(Rect.fromLTWH(0, 0, size.width, size.height)),
    );

    for (int i = 0; i < 160; i++) {
      canvas.drawCircle(
        Offset(((i * 83) % size.width).toDouble(), ((i * 47) % size.height).toDouble()),
        i % 5 == 0 ? 1.8 : .8,
        Paint()..color = cyan.withOpacity(.55),
      );
    }
...
echo "✅ HARD WIRED: logo fixed, Pricing/Contact background added, Marketplace inside Dashboard."
